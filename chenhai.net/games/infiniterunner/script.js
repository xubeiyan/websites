...
使用了Sketch.js库来绘制canvas，具体请参见https://github.com/soulwire/sketch.js

**/

//dirty code made in less the 24 working hours
let scoreColors = ["#181818", '#076C00', '#0300A9', '#9F8F00'];
let playerColor = "#181818";
let gameFont = '微软雅黑';
...
Player.prototype.update = function () {
    ...
    if (this.y > InfiniteRunner.height || this.x + this.width < 0) {
        ...
        InfiniteRunner.scoreColor = scoreColors[0];
...
InfiniteRunner.draw = function () {
    ...
    this.font = `12pt ${gameFont}`;
    this.fillStyle = scoreColors[0];
    this.fillText(`最高分数: ${this.jumpCountRecord}`, this.width - (150 + (this.aceleration * 4)), 33 - (this.aceleration * 4));
    this.fillStyle = this.scoreColor;
    this.font = (12 + (this.aceleration * 3)) + `pt ${gameFont}`;
    this.fillText(`当前分数: ${this.jumpCount}`, this.width - (150 + (this.aceleration * 4)), 50);
